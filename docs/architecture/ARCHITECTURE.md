...
For first use, Sayf first acquires an exclusive transaction on the coordination sidecar. The lock name is a fixed-length hash of the normalized, case-folded resolved target path, conservatively folding aliases that may refer to the same target on case-insensitive filesystems. Recovery authorization uses a separate, stricter fixed-length identity derived from the resolved target using only the operating system's native path normalization. This separation means conservative coordination collisions can serialize distinct case-sensitive targets without allowing one target's pending marker to authorize recovery of the other. Only the lock owner may inspect the target path. If the target is absent, the owner writes the recovery token to a same-directory staging file, flushes and fsyncs it, atomically publishes the target-specific pending marker, and on POSIX fsyncs the containing directory before reserving the target. The owner then reserves the target with exclusive file creation and creates the schema inside one SQLite transaction. If final marker publication or its required durability step fails, the target is not reserved. If the process or host stops after durable marker publication but before commit, the marker survives independently of the target transaction; the next lock owner may recover only a marker-backed target that remains schema-less SQLite storage for that recovery identity. A non-Sayf or user-schema-bearing target is still rejected. Recovery authorization is also revoked durably before initialization reports success: after schema commit, or after validating an already valid ledger that still has a marker, Sayf removes the marker and on POSIX fsyncs its parent directory. An unlink or revocation-sync failure causes initialization to fail even though the ledger may already be locally valid. If a prior revocation sync failed after the marker became absent in the live filesystem, the next idempotent initialization must fsync that absence before it can return successfully. This ordering prevents a contender from classifying another Sayf process's in-progress target as pre-existing storage, avoids stale recovery authorization after successful initialization, and does not require filesystem hard links. Appends pass through the same barrier before the normal `BEGIN IMMEDIATE` append transaction.
...
- crash-recoverable first-use ownership with durable marker publication and durable recovery-authorization revocation, without cross-authorizing distinct targets;
